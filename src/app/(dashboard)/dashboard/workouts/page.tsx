...
export default async function WorkoutsPage() {
  ...
  // Server action: delete a workout session (cascades sets)
  async function deleteSession(formData: FormData) {
    ...
    // Ensure the session belongs to user before deleting
    await prisma.workoutSession.deleteMany({ where: { id, userId: (authUser as any).id } })
            ...
            return (
              ...
              >
                ...
                <div className="mt-4 flex items-center gap-2 relative">
                  <Link href={`/dashboard/workouts/${s.id}`} className="text-[11px] px-3 py-1 rounded-full bg-white/15 hover:bg-white/25 backdrop-blur font-medium">View</Link>
                  <form action={deleteSession} className="inline-flex">
                    <input type="hidden" name="sessionId" value={s.id} />
                    <button type="submit" className="text-[11px] px-3 py-1 rounded-full bg-gradient-to-r from-rose-500 to-pink-600 hover:brightness-110 font-medium shadow focus:outline-none focus-visible:ring-2 focus-visible:ring-rose-200/60">Delete</button>
                  </form>
                </div>
              </div>
            )
